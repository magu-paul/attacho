...
<sql:query var="rs" dataSource="${db}">select st.fname,st.lname,st.school_id,st.depid,stu.staff_no,stu.contact,count(s.regno) as no_student,count(s.marks) as assessed
    from staff st join staff_users stu on st.staff_id = stu.staff_no left outer join stud_users s on s.staff = stu.staff_no group by stu.staff_no</sql:query>
 
<table style="width:auto; border: 2px solid black">
<th style="width:auto; border: 1px solid black">Lecturer Name</th>
<th style="width:auto; border: 1px solid black">Staff Number</th>
<th style="width:auto; border: 1px solid black">Member School</th>
<th style="width:auto; border: 1px solid black">Member Department</th>
<th style="width:auto; border: 1px solid black">Phone Number</th>
<th style="width:auto; border: 1px solid black">Number of Students</th>
<th style="width:auto; border: 1px solid black">Assessed Students</th>
<th style="width:auto; border: 1px solid black">Add Student</th>

 <c:forEach items="${rs.rows}" var="row">

 <tr style="width:auto; border: 1px solid black">
 <td><c:out value="${row.fname}"></c:out> <c:out value="${row.lname}"></c:out></td>
 <td><c:out value="${row.staff_no}"></c:out></td>
 <td><c:out value="${row.school_id}"></c:out></td>
 <td><c:out value="${row.depid}"></c:out></td>
<td><c:out value="${row.contact}"></c:out></td>
<td style="width:auto; border: 1px solid black"><c:out value="${row.no_student}"></c:out></td>
<td style="width:auto; border: 1px solid black"><c:out value="${row.assessed}"></c:out></td>
<td style="width:auto; border: 1px solid black"><form method="post" action="addStudent"><input type="text" placeholder="REG NO" name="regno"><button type = submit name="staff" value=${row.staff_no }>Add</button></form></td>
<td style="width:auto; border: 1px solid black"><form method="post" action="removeStaff"><button type = submit name="staff" value=${row.staff_no }>Remove</button></form></td>
 </tr>
